...
import oak_d_mttqPub_mobnet_20240504 as mttqPub
    ...
    while True:
        ...
        if len(detections) != 0:
            ...
            for detection in detections:
                ...
                locx, locy, locz, confidence = (int(detection.spatialCoordinates.x)), (int(detection.spatialCoordinates.y)), (int(detection.spatialCoordinates.z)), (detection.confidence * 100)

                spacialMobileNetData = [x1, x2, y1, y2, label, locx, locy, locz, confidence]

                # append data to list
                spacialMobileNetDataList.append(spacialMobileNetData)

                print("detection label, labelnum, x, y, z, confidence ...  ", label, detection.label, locx, locy, locz, confidence)

            ### moved the below two lines under  'elif len(detections) == 0:' lines below to publish data in the case of no detections
            # publish imuData over mttq
            # mttqPub.publish_data(spacialMobileNetDataList)

        elif len(detections) == 0:
            spacialMobileNetDataList = [[0, 0, 0, 0, 'None', 0, 0, 0, 0]]
