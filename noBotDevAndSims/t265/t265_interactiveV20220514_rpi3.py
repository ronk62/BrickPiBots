...
#
# 5/14/2022                 - modified for t265 facing up toward ceiling
#                           - robot yaw now = t265 roll
#
        ...
        for i in range(13):
            ...
            try:   # read the sensor - we really only need x, z, and yaw for this
                ...
                if pose:
                    ...
                    # Ryaw = -m.asin(2.0 * (x*z - w*y)) * 180.0 / m.pi;                         # cam pitch
                    Ryaw = m.atan2(2.0 * (w*x + y*z), w*w - x*x - y*y + z*z) * 180.0 / m.pi;  # cam roll
                    # Ryaw   =  m.atan2(2.0 * (w*z + x*y), w*w + x*x - y*y - z*z) * 180.0 / m.pi; # cam yaw
                    ...
                    RSyaw   =  m.atan2(2.0 * (Sw*Sx + Sy*Sz), Sw*Sw - Sx*Sx - Sy*Sy + Sz*Sz) * 180.0 / m.pi;

                    # # print formatted columns of data for easy consumption
                    # if i == 0:
                    #     print()
                    #     print("xT  |  zT  |  Ryaw  |  Confid  |  S result  |  SxT  |  SzT  |  RSyaw")
                    #     print("----   ----   -----   ------     --------     ------  -----   ----")
                    # print(round(xT,2), " ", round(zT,2),  " ", round(Ryaw,2),  "   ", trackerConfidence,  "       ", result,  "     ", round(SxT,2), "   ", round(SzT,2), " ", round(RSyaw,2))


                    # print formatted columns of data for easy consumption
                    if i == 0:
                        print()
                        print("xT  |  yT  |  zT   |  pitch  |  roll  |  yaw  |  Confid  ")
                        print("----   ----   ----    -----     ----     ----    ------")
                    print(round(xT,2), " ", round(yT,2), " ", round(zT,2), " ", round(pitch,2), " ", round(roll,2), " ", round(yaw,2),  "   ", trackerConfidence)
