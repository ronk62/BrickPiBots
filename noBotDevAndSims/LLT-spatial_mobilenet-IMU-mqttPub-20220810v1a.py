...
#
# 8/10/2022                 - updated to publish IMU data via mqtt
#                             this will allow consumption by subscibers, incldg one for live
#                             plotting via matplotlib animation
...
# debug
print("importing time, sys - waiting 1 secs")
import time, sys
time.sleep(1)
...
# debug
print("importing paho.mqtt.client as mqtt - waiting 1 secs")
...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        for imuPacket in imuPackets:
            ...
            roll_xP, yaw_zP, pitch_yP = euler_from_quaternion(cw, cx, cy, cz)  # swapped pitch and roll

            if debugPrint:
                print("RPY [deg]: roll_xP: {0:.7f}, pitch_yP: {1:.7f}, yaw_zP: {2:.7f}".format(roll_xP, pitch_yP, yaw_zP))
            ...
            # ## rotate around x by (minus) -roll_xP # suspected problems negated roll_xP
            # # convert to radians
            # theta1Rad=math.radians(-roll_xP)
            # # ## rotate around x by roll_xP # removed negation
            # # # convert to radians
            # # theta1Rad=math.radians(roll_xP)
            # Rq_01 = Rx(theta1Rad)
            ...
            # ## rotate around y by pitch_yP
            # # convert to radians
            # theta1Rad=math.radians(pitch_yP)
            # Rq_03 = Ry(theta1Rad)
            ...
            # ## rotate around z by yaw_zP
            # # convert to radians
            # theta1Rad=math.radians(yaw_zP)
            # Rq_05 = Rz(theta1Rad)
            ...
            ## mqtt publish data
            try:
                client.publish("topic/imuyaw", yaw_zP);
            except:
                # disconnect mqtt client on error
                client.disconnect();
                exit

            # ## code to test plotting locally - does not work after adding mqtt code
            # ## (uncomment next section as needed for dev/test, but first, 
            # ## comment all mqtt code first)
            # #yaw_zP = 45   # for testing
            # yaw_zP2 = yaw_zP

            # # calculate 3d compass "needle" endpoints
            # headingX0 = 1
            # headingY0 = 1
            # headingX1 = headingX0 + math.cos(math.radians(yaw_zP2))
            # print("headingX1 = ", headingX1)
            # headingY1 = headingY0 + math.sin(math.radians(yaw_zP2))
        ...
        # print checkpoint for dev/test
        print("completed mqtt publish data section... ")
