...
cvNet = cv.dnn.readNetFromTensorflow('/home/robot/ev3dev2Projects/noBotDevAndSims/murtaza/cv2_dnn_object_detection/frozen_inference_graph.pb', '/home/robot/ev3dev2Projects/noBotDevAndSims/murtaza/cv2_dnn_object_detection/ssd_mobilenet_v2_coco_2018_03_29.pbtxt')

# img = cv.imread('/home/robot/ev3dev2Projects/noBotDevAndSims/murtaza/cv2_dnn_object_detection/example.jpg')
...
while True:
    ...
    for detection in cvOut[0,0,:,:]:
        ...
        if score > 0.3:
            ...
            # for testing
            # print("type class_id", type(class_id),"detection[2]", type(detection[2]),"class_label", type(class_label))
            if class_label == None:
                class_label = 'noneFound'
            # for testing
            # print("type class_id", type(class_id),"detection[2]", type(detection[2]),"class_label", type(class_label))
            # print(str(str(class_id) + " " + str(detection[2])  + " " + class_label))
            # for testing
            # print("int(top), int(bottom), class_label  =  ", int(top), int(bottom), class_label)
            print("class_label:", class_label, "vertical size in pixels:", int(bottom) - int(top))
