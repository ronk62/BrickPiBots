...
#
# 1/3/2021      Ron King    - based on BPDDwasdUS_IRdataAcqPltV9.22b, this version is a major rewrite for a new
#                             IR localization model using a single, rotating beacon. 2 beacons produce
#                             too much noise from channel cross-talk
#                           - stubbed out ir2, b2lock, beacon2 related code
#                           - removed the section of code "push g" for manual invoked graphing
#                           - changed robot seek motion and strategy from continuous motion to incremental
        ...
        if sample_mode > 0:
            try:
                if beaconCali_mode == 0:
                    ...
                    ### comment the following section for quick IR testing
            ...
            # print("Sampling complete. Returning to keyboard cmd mode.")
                ...
                for i in range(1,cycleCount):
                    ...
                    while not b1lock:
                        ...
                        # take samples for 10 seconds (to ensure beacon rotates 1 full turn)
                        ...
                        if not b1lock:
                            if b1IRdistMean > 0 and b1IRhStdev < 300 and b1IRhMean < -20:
                                ...
                                hailmarryTimeout = time.time()
                            elif b1IRdistMean > 0 and b1IRhStdev < 300 and b1IRhMean < 0:
                                # move ccw 10 deg
                            ...
                            if b1IRdistMean > 0 and b1IRhStdev < 300 and b1IRhMean > 20:
                                ...
                                # move cw 10 deg
                                mL.on(1, brake=False)
                                print("moving cw 10 deg; b1IRdistMean, b1IRhMean, b1IRhStdev", b1IRdistMean, b1IRhMean, b1IRhStdev)
                                mL.on_for_degrees(speed=7, degrees=45)
                                hailmarryTimeout = time.time()

                            # if we don't see a valid +25 or -25 heading, turn 45 deg incr until we do
                            ...
                            if (b1IRdistMean > 300 or b1IRdistMean == -1) and b1IRhMean == 0:
                                # move 45 deg in cw direction until we pick up a beacon signal
                                print("moving 45 deg in cw direction; b1IRdistMean, b1IRhMean", b1IRdistMean, b1IRhMean)
                                mL.on_for_degrees(speed=21, degrees=202)
                                hailmarryTimeout = time.time()
                            ...
                            if b1IRdistMean > 0 and b1IRdistMean < 300 and b1IRdistStdev < 3 and b1IRhMean >= -0.7 and b1IRhMean <= 0.7 and b1IRhStdev < 3:
                                ...
                                ### don't collect new ir data; the beacon is rotating (1/3/2021)
                                # init the data collection arrays for each collection series
                                cmpDeg = np.array([], dtype=np.int32)
                                # ir1r = np.array([], dtype=np.int32)
                                ...
                                zeroir1hdist = b1IRdistMean
                                zeroir1hdistStdev = b1IRdistStdev
            ...
            plt.title('IR beacons - robot frame relative position data V20210103')
            plt.legend()
            # plt.show()
            ...
            plt.xlabel('x-position')
            plt.ylabel('y-position')
            plt.title('US & compass point cloud data V20210103')
            plt.legend()
            # plt.show()
            ...
            plt.xlabel('x-position')
            plt.ylabel('y-position')
            plt.title('IR beacons - world frame position data V20210103')
            plt.legend()
            plt.show()
