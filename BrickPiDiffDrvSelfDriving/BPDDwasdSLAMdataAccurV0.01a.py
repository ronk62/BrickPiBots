...
# 6/16/2020     Ron King    - used file /home/robot/ev3dev2Projects/BrickPiDiffDrvSelfDriving/BPDDwasdSLAMtestV0.01a.py as starting point
#                           - this program will allow collection of US distance data at specific distances and angle for
#                           statistical analysis (min, max, avg, stdev) ... US data accuracy testing
...
### Main Loop
    while (True):
        ...
        if x == 112: # p pushed - print sample statistics to screen
            if polarCoordsUSr.size < 1:
                print("no data in polarCoordsUSr at this time")
            else:
                ...
                print("")
            if polarCoordscmpDeg.size < 1:
                print("no data in polarCoordscmpDeg at this time")
            else:
        ...
        if sample_mode > 0:
            ...
            if polarCoordsUSr.size > 10000:
                sample_mode *= -1
                print("")
                print("")
                print ("Exiting sample mode due to sample size...")
                print ("polarCoordsUSr.size = ", polarCoordsUSr.size)
                print("min      max      mean      std")
                print(np.min(polarCoordsUSr), np.max(polarCoordsUSr), np.mean(polarCoordsUSr), np.std(polarCoordsUSr))
                print("")
