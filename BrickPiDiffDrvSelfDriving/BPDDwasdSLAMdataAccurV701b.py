...
# 7/28/2020     Ron King    - used file /home/robot/ev3dev2Projects/BrickPiDiffDrvSelfDriving/BPDDwasdSLAMdataAccurV601b.py as starting point
...
#                           - primary purpose of this version is to gather min, max, avg IR-beacon distance measurements
#                           - manual movement to location then park and gather data samples to arrays
#                           - print statistics to screen and/or save to files
# 7/30/2020     Ron King    - irDistVal is now set to int(3.19 * ir.distance()) to approximate distance in cm
#
    ...
    while (True):
        ...
        if x == 102: # f pushed - save sample data to files
            ...
            if polarCoordsUSr.size < 1 or polarCoordscmpDeg.size < 1 or polarCoordsIRr.size < 1 or ODmRencoderVal.size < 1:
                ...
                # save to csv files
                np.savetxt('polarCoordsUSrV701b.csv', polarCoordsUSr, delimiter=',')
                np.savetxt('polarCoordsIRrV701b.csv', polarCoordsIRr, delimiter=',')
                np.savetxt('ODmRencoderValV701b.csv', ODmRencoderVal, delimiter=',')
                np.savetxt('polarCoordscmpDegV701b.csv', polarCoordscmpDeg, delimiter=',')
        ...
        if sample_mode > 0:
            try:
                ...
                # take 1000 samples while parked at location
                for i in range(1000):
                    usDistCmVal = us.distance_centimeters
                    irDistVal = ir.distance()
                    if irDistVal == None:
