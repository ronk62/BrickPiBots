...
# 6/21/2020     Ron King    - used file /home/robot/ev3dev2Projects/BrickPiDiffDrvSelfDriving/BPDDwasdSLAMdataAccurV0.01a.py as starting point
...
#
# 6/27/2020     Ron King    - added IR sensor (proximity mode) so that measurement data could be compared to US Dist
#
...
# SLAM vars
polarCoordsUSr = np.array([], dtype=np.int32)       # list to hold 'r' - US distance r (in cm)
polarCoordscmpDeg = np.array([], dtype=np.int32)    # list to hold 'thetaDeg' - compass-angle  in degrees
polarCoordsUSmean = np.array([], dtype=np.int32)    # list to hold processed data US-mean
polarCoordsUSstd = np.array([], dtype=np.int32)     # list to hold processed data US-stdev
polarCoordscmpMean = np.array([], dtype=np.int32)   # list to hold processed data cmps-mean
# added IR
polarCoordsIRr = np.array([], dtype=np.int32)       # list to hold 'r' - IR distance r (in arb units)
polarCoordsIRmean = np.array([], dtype=np.int32)    # list to hold processed data IR-mean
polarCoordsIRstd = np.array([], dtype=np.int32)     # list to hold processed data IR-stdev
...
### Main Loop
    while (True):
        ...
        if usDistCmVal != prev_usDistCmVal:
            if printVerbose > 0:
                print("usDistCmVal = ", int(usDistCmVal), "  irProxVal = ", irProxVal, "  compassVal = ", compassVal)  # print all sensor vals, regardless of which changed
        ...
        if x == 112: # p pushed - print sample statistics to screen
            if polarCoordsUSmean.size < 1 or polarCoordsUSstd.size < 1 or polarCoordsIRmean.size < 1 or polarCoordsIRstd.size < 1 or polarCoordscmpMean.size < 1:
                ...
                print("")
                print ("polarCoordsIRmean.size = ", polarCoordsIRmean.size)
                print("")
                print ("polarCoordsIRmean = ", polarCoordsIRmean)
                print("")
                print ("polarCoordsIRstd.size = ", polarCoordsIRstd.size)
                print("")
                print ("polarCoordsIRstd = ", polarCoordsIRstd)
                print("")
        ...
        if x == 102: # f pushed - save sample statistics to file
            ...
            if polarCoordsUSmean.size < 1 or polarCoordsUSstd.size < 1 or polarCoordsIRmean.size < 1 or polarCoordsIRstd.size < 1 or polarCoordscmpMean.size < 1:
                ...
                # save to csv files
                np.savetxt('polarCoordsUSmean.csv', polarCoordsUSmean, delimiter=',')
                np.savetxt('polarCoordsUSstd.csv', polarCoordsUSstd, delimiter=',')
                np.savetxt('polarCoordsIRmean.csv', polarCoordsIRmean, delimiter=',')
                np.savetxt('polarCoordsIRstd.csv', polarCoordsIRstd, delimiter=',')
                np.savetxt('polarCoordscmpMean.csv', polarCoordscmpMean, delimiter=',')
        ...
        if x == 114: # r pushed - reinitializing arrays
            ...
            polarCoordsIRmean = np.array([], dtype=np.int32)
            polarCoordsIRstd = np.array([], dtype=np.int32)
            ...
            try:
                ...
                for i in range(360):
                    ...
                    for j in range(100):
                        ...
                        polarCoordsIRr = np.append(polarCoordsIRr, irProxVal)
                    ...
                    polarCoordsIRmean = np.append(polarCoordsIRmean, np.mean(polarCoordsIRr))
                    polarCoordsIRstd = np.append(polarCoordsIRstd, np.std(polarCoordsIRr))
