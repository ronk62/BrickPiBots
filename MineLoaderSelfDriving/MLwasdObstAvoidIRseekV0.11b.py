...
#
# 5/24/2020     Ron King    Removed the beacon search routine - as previously implemented, it was redundant 
#                           Recontructed the compass search routine to search only northerly
#                           Made several failed attempts to harmonize Med obstacle avoidance.
#                           NOT CIRRENTLY WORKING CORRECTLY
#
...
compassVal = 0
prev_compassVal = 0
compassGoal = 275     # with ht-compass mounted sideways, this translates to mostly North, slightly East
...
### Main Loop
    while (True):
        ...
        if pilot_mode < 0:
            if  obstacleNear == True:
                ...
                if evadeLeft == True:
                    ...
                    turnRatio = -0.5
                if (evadeLeft or evadeRight) and (time.time() - evadeTime >= 5):
                        spd = 0
                        turnRatio = 0
                        mL.on(spd, brake=False)
                        mR.on(spd, brake=False)
                ...
                if not (evadeLeft or evadeRight):
                    ...
                    evadeTime = time.time()
                # determine next action based on compass orientaion
                if (compassVal >= (compassGoal)) or (compassVal < (compassGoal - 270)):
                    evadeLeft = True
                elif (compassVal < (compassGoal)) or (compassVal >= (compassGoal - 90)):
                    evadeRight = True
                spd = 5
                if evadeLeft == True:
                    turnRatio = 0.5
                    print("Doing Med obstacle avoidance - turing Left")
                elif evadeRight == True:
                    turnRatio = -0.5
                    print("Doing Med obstacle avoidance - turing Right")
        ...
        if navPrint == 10:
            ...
            print("NAV:  x, spd, turnRatio, pilot_mode, cmpSearch, cmpGenHeading, beaconLock  ", x, spd, turnRatio, pilot_mode, cmpSearch, cmpGenHeading, beaconLock)
            navPrint = 0
        navPrint += 1
